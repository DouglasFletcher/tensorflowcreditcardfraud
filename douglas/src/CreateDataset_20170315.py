...
class CreateDataset:
	...
	for now use basic neural network,
	need to create binary variables
	based on given input data & DataInvestigate
	outcomes.
	"""
	@classmethod
	...
	def __createTransformations(self):
		"""
		create transformations: based on visualizations fraud transactions 
		distributions in some variables are either more left or right skewed.
		This can be used as justification to find instances which differentiate
		fraud vs non-fraud transactions as categorical variables. This can be fed 
		to the first perceptons layer of the neural network.
		"""
